      ...
      {claims.length ? (
        <div>
          <div className={container}>
            <p className={noMargin}>title : {JSON.stringify(claims[num].text)}</p>
            <p className={noMargin}>RESULT: {JSON.stringify(claims[num]?.claimReview[0].textualRating)}</p>
            <p className={noMargin}>Description : {JSON.stringify(claims[num]?.claimReview[0].title)}</p>
          </div>
